...
if __name__ == '__main__':
    ...
    for i in range(N):
        ...
        fig.savefig(os.path.join(general_figs_folder, 'local-SHAP', local_plt_name), bbox_inches='tight')
        ...
        for key, value in interval_shap_values.items():
            ...
            ax.set_title(f"Output explanation, interval [{key}]   ", fontsize=round(8*(DPI_RES/72)))

            fig.savefig(os.path.join(interval_figs_folder, local_interv_plt_name),
                        bbox_inches='tight', dpi=DPI_RES)
            fig.savefig(f'temp_plot_{i}_{key}.png',
                        bbox_inches='tight', dpi=DPI_RES)
            # plt.show()
            plt.close(fig)  # Close the figure to free up memory

            print(f'TIME INTERVAl: {key}')
            print(f'Local prediction auto: {shap_values_use.base_values + shap_values_use.values.sum():.4f}')
        ...
        try: #newer Pillow >=11.3.0
            ...
            text_width, text_height = draw.textsize(title_text, font=font)  # older Pillow
